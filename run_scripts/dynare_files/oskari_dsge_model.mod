% dsge model with shadow banking and altered parameters

% variables 

var y c pi inv k q int rd rb p lab In R Lambda varho g Z pi_dot Dp delta U F ym phi w
Nn z nw Nex x ene veta sb d	b ssb rsb v qa fs Theta shock_A shock_FS shock_G shockvar_Lambda shock_Ne shock_XI shock_MU;

% exogenous variables (shock sigmas)

varexo e_A e_FS e_G e_Lambda e_Ne e_XI e_MU e_I;

% parameters

parameters cbeta ch cchihh cxi cfrisch comega ctheta calpha cdelta cepsilon cgamma cgammap czeta cs cb comegahh cchisb 
ceta I_ss g_ss int_ss pi_ss phi_int phi_y phi_pi rho_A rho_FS rho_XI rho_MU rho_G rho_Lambda LambdaBAR rho_Ne; 

% re-evaulauted parameters
calpha = 0.375;         % capital share of output, originally 0.33
cdelta = 0.035;         % depreciation parameter, originally 0.02071939
ctheta = 0.9775;        % bank survival probability, originally 0.972
cchisb = 0.078;         % shadow bank sepratation rate, originally 0.1
...
comega = .002;		    % proportional starting up funds
cchihh = 3.409;		    % labour utility weight
...
int_ss = 0.0100501;     % steady state of monetary policy
pi_ss = 0;           % steady state of inflation

phi_int = 0.8177;       % interest rate inertia towards target rate (smoothing parameter) 
phi_y = 0.8268;         % output gap parameter
phi_pi = 1.7584;        % inflation parameter towards targeted inflation
...
% 11. monetary policy, Taylor rule PBoC target inflation

exp(int) = exp(int_ss)^(1-phi_int)*exp(int(-1))^phi_int*((exp(pi)/exp(pi_ss))^phi_pi*(exp(y)/exp(y(-1)))^phi_y)^(1-phi_int)*exp(e_I);
...
stoch_simul(order=1, irf=40, nograph) c k y pi lab b int d fs rsb ssb U delta sb inv rb q rd Nn nw Nex R varho;